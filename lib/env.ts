...
const EnvSchema = z.object({
  ...
  AUTH_MODE: z.enum(['development-bypass', 'proxy-header']).default('development-bypass'),
  AUTH_PROXY_EMAIL_HEADER: z.string().min(1).default('x-opentalent-user-email'),
  AUTH_PROXY_NAME_HEADER: z.string().min(1).default('x-opentalent-user-name'),
  AUTH_PROXY_ROLE_HEADER: z.string().min(1).default('x-opentalent-user-role'),
  AUTH_DEV_USER_EMAIL: z.string().email().default('local.admin@airnav.co.id'),
  AUTH_DEV_USER_NAME: z.string().min(1).default('Local Admin'),
  AUTH_DEV_USER_ROLE: z.enum(['viewer', 'manager', 'admin']).default('admin'),
